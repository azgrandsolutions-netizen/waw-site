      ...
      <div className="mx-auto mb-4 w-[min(100%-1rem,900px)] rounded-2xl border bg-white/90 p-4 shadow-xl backdrop-blur">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm leading-6">
            Folosim cookie-uri pentru analiză. Implicit sunt <b>dezactivate</b> (Consent Mode v2).
          </p>
          <div className="flex gap-2">
            <button className="rounded-xl border px-3 py-2 text-sm" onClick={() => setConsent("denied")}>
              Respinge
            </button>
            <button className="rounded-xl bg-black px-3 py-2 text-sm text-white" onClick={() => setConsent("granted")}>
              Acceptă
            </button>
          </div>
        </div>
